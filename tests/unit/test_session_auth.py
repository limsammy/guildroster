...
class TestSessionAuth:
    ...
    @patch("app.utils.session_auth.get_db")
    def test_get_session_from_cookie_session_not_found(self, mock_get_db):
    ...
    def test_get_session_from_cookie_invalid_session(self, mock_get_db):
        """Test getting session when session is invalid."""
        user = self.setup_user()
        session = self.setup_session(user)
        session.is_active = False  # type: ignore # Make session invalid
    ...
    @patch("app.utils.session_auth.get_current_user")
    ...
    @patch("app.utils.session_auth.require_user")
