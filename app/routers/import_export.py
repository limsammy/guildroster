    ...
    try:
        ...
        if file.filename.endswith('.zip'):
                    ...
                    if file_info.filename.endswith('.json'):
                        with zip_file.open(file_info.filename) as json_file:
                            ...
                            if 'id' in json_data and 'data' in json_data:
                                ...
                                data[file_info.filename.replace('.json', '')] = json_data
        else:
            ...
            try:
                ...
                if isinstance(json_data, dict) and not ('id' in json_data and 'data' in json_data):
                    # Multi-resource format
                    data = {}
                    for key, resource_data in json_data.items():
                        if isinstance(resource_data, dict) and 'id' in resource_data and 'data' in resource_data:
...
async def process_import_data(data: Dict[str, Any], db: Session) -> Dict[str, Any]:
    """
    ...
    if "guilds" in data:
        for guild_data in data["guilds"]:
            try:
                ...
                results["guilds"]["errors"].append(f"Guild {guild_data.get('name', 'Unknown')}: {str(e)}")
        ...
        for team_data in data["teams"]:
            try:
                ...
                if "guild_id" not in team_data and "guild_name" in team_data:
                    guild = db.query(Guild).filter(Guild.name == team_data["guild_name"]).first()
                    if guild:
                        team_data["guild_id"] = guild.id
                    else:
                        results["teams"]["errors"].append(f"Team {team_data.get('name', 'Unknown')}: Guild not found")
                        continue
                ...
                results["teams"]["errors"].append(f"Team {team_data.get('name', 'Unknown')}: {str(e)}")
    ...
    if "scenarios" in data:
        for scenario_data in data["scenarios"]:
            try:
                ...
                results["scenarios"]["errors"].append(f"Scenario {scenario_data.get('name', 'Unknown')}: {str(e)}")
        ...
        for toon_data in data["toons"]:
            try:
                ...
                if "team_id" not in toon_data and "team_name" in toon_data:
                    team = db.query(Team).filter(Team.name == toon_data["team_name"]).first()
                    if team:
                        toon_data["team_id"] = team.id
                    else:
                        results["toons"]["errors"].append(f"Toon {toon_data.get('name', 'Unknown')}: Team not found")
                        continue
                ...
                results["toons"]["errors"].append(f"Toon {toon_data.get('name', 'Unknown')}: {str(e)}")
        ...
        for raid_data in data["raids"]:
            try:
                ...
                if "team_id" not in raid_data and "team_name" in raid_data:
                    team = db.query(Team).filter(Team.name == raid_data["team_name"]).first()
                    if team:
                        raid_data["team_id"] = team.id
                    else:
                        results["raids"]["errors"].append(f"Raid {raid_data.get('scheduled_at', 'Unknown')}: Team not found")
                        continue
                ...
                results["raids"]["errors"].append(f"Raid {raid_data.get('scheduled_at', 'Unknown')}: {str(e)}")
...
def get_export_status():
    ...
    return {
        ...
        "supported_data_types": ["guilds", "teams", "toons", "raids", "scenarios"]
    }
