...
![Schema](./guildroster_schema_v0.4.png)
...
- **Invites**:  
  Stores invite codes generated by superusers for new user signups. Tracks which superuser created the invite (`created_by`), which user redeemed it (`used_by`), creation and usage timestamps, and whether the invite is still active.

- **Users**:  
  Stores user account information, including username, password, invite code used for signup, superuser status, and the guilds they belong to. Tracks creation and modification timestamps. The `invite_code` field links to the code used from the Invites table.

- **Guilds**:  
  Represents a guild, with a name and relationships to its members and teams. Each guild can have multiple members and teams. Tracks creation and modification timestamps.

- **Members**:  
  Represents individual guild members, storing their display name and Discord username. Each member can have multiple toons (characters). Tracks creation and modification timestamps.

- **Toons**:  
  Represents a character (toon) belonging to a member. Each toon has a username and a flag indicating if it is the member’s main character. Toons are linked to attendance records for raids. Tracks creation and modification timestamps.

- **Teams**:  
  Represents a team within a guild, with a name and relationships to its members and raids. Teams are used to organize members for specific raid groups or activities. Tracks creation and modification timestamps.
...
- **Attendance**:  
  Tracks which toons attended which raids, and whether they were present or absent. Tracks creation and modification timestamps.

- **Scenarios**:  
  Lookup table (or enum) representing different raid scenarios or instances (e.g., specific dungeons or raid zones). Tracks creation and modification timestamps.
